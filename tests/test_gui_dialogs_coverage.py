...
class TestAddTodoDialogLocation:
    """Location field round-trip from AddTodo's Advanced disclosure."""

    def test_advanced_field_round_trip(self, app):
        dialog = AddTodoDialog()
        dialog._on_toggle_advanced()
        dialog.reminder_edit.setText("Standup")
        dialog.location_edit.setText("Conference Room B")
        dialog._on_accept()
        item = dialog.get_item()
        assert item is not None
        assert item.location == "Conference Room B"

    def test_smart_input_path_picks_up_location(self, app):
        # Even without entering Advanced mode, if the user typed in
        # the location_edit (e.g. via Advanced briefly opened earlier),
        # the smart-input path applies that value to the constructed item.
        dialog = AddTodoDialog()
        dialog._smart_input.set_text("Buy groceries tomorrow")
        dialog.location_edit.setText("Store on Main St")
        dialog._on_accept()
        item = dialog.get_item()
        assert item is not None
        assert item.location == "Store on Main St"

    def test_empty_location_default(self, app):
        dialog = AddTodoDialog()
        dialog._smart_input.set_text("Buy groceries")
        dialog._on_accept()
        item = dialog.get_item()
    ...
    def test_location_whitespace_stripped(self, app):
        dialog = AddTodoDialog()
        dialog._on_toggle_advanced()
        dialog.reminder_edit.setText("Standup")
        dialog.location_edit.setText("  Conference Room B  ")
