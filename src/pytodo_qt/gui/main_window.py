...
class MainWindow(QMainWindow):
    ...
    def _connect_table_signals(self) -> None:
        ...
        self.todo_table.create_list_requested.connect(self._on_add_list)
    ...
    def _connect_kanban_signals(self) -> None:
        ...
        self.kanban_board.create_list_requested.connect(self._on_add_list)
    ...
    def _on_item_due_time_end_changed(self, item_id: UUID, due_time_end) -> None:
        """Handle item due time end change."""
        if self._refreshing:
            return
        active_list = self._database.active_list
        if active_list:
            item = active_list.get_item(item_id)
            if item:
                from .commands import EditDueTimeEndCommand
